...
function MeetingTimeDateSelection({eventInfo,businessInfo}) {
    ...
    const handleDateChange=(date)=>{
        ...
        console.log(`Is ${day} available?`, Boolean(businessInfo?.daysAvailable?.[day]));
    ...
    const getPrevEventBooking = async (date_) => {
      ...
      querySnapshot.forEach((doc) => {
          ...
          if (doc.data()) {
              setPrevBooking(prev => {
                  // Check if prev is not null or undefined before spreading, else start with an empty array
                  const updatedPrev = prev ? [...prev] : [];
                  updatedPrev.push(doc.data());
                  return updatedPrev;
              });
          }
      });
